import get from './get';
import { saveCSS, cacheLatest } from '../storage';
...
const zhd = {
    ...
    // 长评论更多
    longcommentsmore: `${host}/api/${version}/story/:id/long-comments/before/:lastid`,
    // 短评论
    shortcomments: `${host}/api/${version}/story/:id/short-comments`,
    // 短评论更多
    shortcommentsmore: `${host}/api/${version}/story/:id/short-comments/before/:lastid`,
    // 合集
...
export const longcommentsmore = (id, lastid) => get(replace(zhd.longcommentsmore, [':id', ':lastid'], [id, lastid]));
export const shortcomments = id => get(replace(zhd.shortcomments, ':id', id));
export const shortcommentsmore = (id, lastid) => get(replace(zhd.shortcommentsmore, [':id', ':lastid'], [id, lastid]));
